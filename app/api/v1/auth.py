...
def login_api(
    login_in: LoginIn, request: Request, db: Session = Depends(get_db)
):
...
@router.post("/refresh", summary="刷新token", response_model=SuccessResponse[RefreshOut])
...
def logout_api(logout_in: LogoutIn):
    logout_service(logout_in.refresh_token)
